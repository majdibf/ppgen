...
/**
 * MinIO (S3-compatible) implementation of {@link StoragePort}.
 */
@Slf4j
@ApplicationScoped
public class MinioStorageAdapter implements StoragePort {
    ...
    private void uploadObject(String bucket, String objectKey, InputStream inputStream, String contentType) throws Exception {
        try {
            ...
            log.info("Uploaded object %s to bucket %s", objectKey, bucket);
        } catch (Exception e) {
            log.error("Failed to upload object %s to bucket %s: %s", objectKey, bucket, e.getMessage());
            throw e;
        }
    }
    ...
    private InputStream downloadObject(String bucket, String objectKey) throws Exception {
        try {
            ...
            log.error("Failed to download object %s from bucket %s: %s", objectKey, bucket, e.getMessage());
            throw e;
        }
    }
    ...
    private void deleteObject(String bucket, String objectKey) throws Exception {
        try {
            ...
            log.info("Deleted object %s from bucket %s", objectKey, bucket);
        } catch (Exception e) {
            log.error("Failed to delete object %s from bucket %s: %s", objectKey, bucket, e.getMessage());
            throw e;
        }
    }
